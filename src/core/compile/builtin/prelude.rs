...
impl BuiltinSource for Prelude {
                ...
                Ty::new_function(
                    ...
                    Ty::Transformed(
                        ...
                        Transformation::new(|mut expr| {
                            ...
                            let value_unwrapped =
                                match1!(&value.obj, ExpressionObj::BorrowImmut(value) => &**value);
    ...
    fn static_attr(&self, attr: &String) -> Option<Ty> {
        match (self, attr.as_str()) {
            (Self::Pubkey, "find_program_address") => Some(
                Ty::new_function(
                    vec![
                        ("seeds", Ty::python(Python::List, vec![Ty::Cast(Ty::prelude(Self::Seed, vec![]).into())]), ParamType::Required),
                        ("program_id", Ty::prelude(Self::Pubkey, vec![]), ParamType::Required)
                    ],
                    Ty::Transformed(
                        Ty::python(Python::Tuple, vec![
